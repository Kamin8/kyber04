...
ob_start();
define('API_KEY','5587131247:AAFahVZ4E93d_tMx5sf8jjRA5NST5p-iLqM');
...
$ssa = json_decode(file_get_contents('yuklandi.json'),1);
    $status = bot('getChatMember',['chat_id'=>'@MASTER_PHP_1','user_id'=>$from_id])->result->status;
        if($status == 'left'){
            bot('sendMessage',[
                'chat_id'=>$chat_id,
                'text'=>"Kanalga a`zo bo`ling  🚫' @MASTER_PHP_1",
...
if($text != null){
     if($text == '/start'){
        bot('sendMessage',[
                'chat_id'=>$chat_id,
                'document'=>"https://play.google.com/",
                'parse_mode'=>'markdown',
...
👨‍✈Tizim Havfsizligiga*  [@Xacker_07_UZ] *Javobgar
♻️Guruh:* [@superdasturchilar]
👨‍🔧Admin: Sheroz Saparboyev
Agar botizimini ishlatishini bilmasangiz videoni diqqatlik bilan tomosha qiling /video ',
                'reply_markup'=>json_encode([
              'inline_keyboard'=>[
                  [['text'=>"📢Kanalga azo bo'ling", 'url'=>"https://t.me/uzdastur_ru"]],
              ]
          ])
        ]);
